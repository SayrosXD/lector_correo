...
// Procesar el formulario cuando se hace la petición POST
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $correo_receptor = $_POST['correo.automatizado.yallegue@gmail.com'] ?? '';
    $password_app = $_POST['omhn sbzo ycra nwry'] ?? '';
        ...
        if (!$inbox) {
            ...
            if ($emails) {
                ...
                foreach ($ultimos_correos as $id_correo) {
                    ...
                    $resultado .= "<p class='text-sm text-gray-500'><b>Fecha:</b> " . $overview[0]->date . "</p>";
                    $resultado .= "<p class='text-base font-semibold text-gray-800 mt-1'><b>Asunto:</b> " . htmlspecialchars($overview[0]->subject) . "</p>";
                    ...
                    $resultado .= "<div class='mt-3'><span class='inline-block px-3 py-1 bg-indigo-100 text-indigo-800 text-xs font-bold rounded-full'>Analizador de código: " . $codigo_detectado . "</span></div>";
                    $resultado .= "</div>";
                }
                
                $resultado .= "</div>";
            } else {
                $resultado = "<div class='p-4 bg-amber-50 text-amber-800 rounded-lg border border-amber-200'>Conectado correctamente a la cuenta, pero no se encontraron correos en el INBOX que vengan de <b>{$remitente_objetivo}</b>.</div>";
            }
...
<body class="bg-gray-100 min-h-screen font-sans flex items-center justify-center p-4">
    ...
    <div class="w-full max-w-2xl bg-white rounded-xl shadow-md p-6 md:p-8">
        ...
        <p class="text-sm text-gray-500 mb-6">Este script leerá las alertas entrantes buscando mensajes específicos de <code class="bg-gray-100 px-1 py-0.5 rounded text-red-600">sayros364@gmail.com</code>.</p>
